...
const StockItemDetails = () => {
  ...
  const handleDelievered = (id) => {
    const windowpopup = window.confirm(
      " Are you sure you want to Deliever this item?"
    );
    if (windowpopup) {
      fetch(`http://localhost:5000/stocksItem/${id}`, {
        ...
        .then((data) => {
          console.log(data);
          const remaining = stockItems.filter(
  ...
  return (
    <div>
      <div className="w-25 mx-auto">
        ...
        >
          <input
            className="mb-2 border border-dark border-1"
      ...
      <div className="w-25 mx-auto mt-5">
        {stockItems.map((stockItem) => (
          <div key={stockItem._id}>
            ...
            <button
              onClick={() => handleDelievered(stockItem._id)}
